...
public:
  ...
  const svec<int>& getConsMembers(int readIdx) const { return m_consReads.getConsMembers(readIdx);      }
  ...
  void findOverlaps(int numOfThreads, int mode, double identThresh, int maxOverlapPerRead, float portion=1,  string groupedReadInfo="");  
