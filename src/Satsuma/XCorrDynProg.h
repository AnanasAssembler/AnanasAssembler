...
class XCDynProg
{
 public:
  ...
  double Align(const MultiProtein & target, 
	       const MultiProtein &query, 
	       const SignalFilter & filter,
	       double cutoff = 0.,
	       ostream & o = cout);


  int GetQueryStart() const  {return m_alignment.getQueryOffset();}
  int GetQueryStop() const   {return m_alignment.getQueryOffset()+m_alignment.getQueryBaseAligned();}
  int GetTargetStart() const {return m_alignment.getTargetOffset();}
  int GetTargetStop() const  {return m_alignment.getTargetOffset()+m_alignment.getTargetBaseAligned();}
  double GetIdentity() const {return m_alignment.calcIdentityScore();}
  double GetPVal() const     {return m_alignment.calcPVal();}


 private:

  bool FindBestBracket(int & lastFrame, int & m, double & score);
