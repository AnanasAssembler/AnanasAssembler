...
int main( int argc, char** argv )
{
  ...
  commandArg<bool> tCmmd("-t","target sequence", false);
  commandArg<bool> allCmmd("-a","print entire sequences", false);
  commandArg<int> slackCmmd("-s","flanks on each side", 0);
  commandLineParser P(argc,argv);
  P.SetDescription("Converts a satsuma_summary file into a fasta");
  ...
  bool bAll = P.GetBoolValueFor(allCmmd);
