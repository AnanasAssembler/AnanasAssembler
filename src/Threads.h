...
bool FindOverlapsThread<SuffixType>::OnDo(const string & msg) {
  ...
  for(int i=m_fromIdx; i<m_toIdx; i++) { 
    int overlapThresh = m_runThresh;
    if(overlapThresh==0) { overlapThresh = m_subreads.getReadSize(i)*2; }
    if(m_overlaps[i].getNumLaps()<overlapThresh) {
       m_subreads.findOverlaps(i, m_overlaps, m_mode, overlapThresh); 
     }
     progCount++;
     if (progCount % inc == 0) 
...
bool FindOverlapsSingleThread<SuffixType>::OnDo(const string & msg) {
  ...
  int currIdx = this->m_threadQueue.getNext();
   while(currIdx>=0) {
     int overlapThresh = this->m_runThresh;
     if(overlapThresh==0) { overlapThresh = this->m_subreads.getReadSize(currIdx)*2; }
     if(this->m_overlaps[currIdx].getNumLaps()<overlapThresh) {
       FILE_LOG(logDEBUG3) << "Finding overlaps for read idx: " << currIdx; 
       this->m_subreads.findOverlaps(currIdx, this->m_overlaps, this->m_mode, overlapThresh); 
     }
     if (currIdx  % inc == 0) 
