...
int main( int argc, char** argv )
{
  ...
  while (parser.ParseLine()) {
	...
	continue;
      ...
      if (full_anti > 0) {
	cout << name << "\t" << "FULL_ANTI" << endl;
	continue;
      }
      if (part_anti > 0) {
	cout << name << "\t" << "PARTIAL_ANTI" << endl;
	continue;
      }
      if (intron_sense > 0) {
	cout << name << "\t" << "INTRONIC_SENSE" << endl;
	continue;
      }
      if (intron_anti > 0) {
	cout << name << "\t" << "INTRONIC_ANTI" << endl;
	continue;
      }
      if (other_sense > 0) {
	cout << name << "\t" << "OTHER_SENSE" << endl;
	continue;
      }
      if (other_anti > 0) {
	cout << name << "\t" << "OTHER_ANTI" << endl;
	continue;
      }
